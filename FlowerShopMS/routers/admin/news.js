...
router.get("/",function(req,res,next){
	...
	mssql.sql("select count(*) tot from news,newstype type where news.cid = type.id",function(err,data){
		if (err) {
			return "";
		}else{
			let tot = data[0].tot;
			...
			mssql.sql("select news.*,type.name tname from news,newstype type where news.cid = type.id order by news.id desc limit ?,?",[fpage.start,fpage.size],function(err,data){
				if (err) {
					return "";
				}else{
					data.forEach(item=>{
...
router.post("/add",upload.single("img"),function(req,res,next){
	...
	mssql.sql("insert into news(title,keywords,description,info,author,cid,text,num,time,img) value(?,?,?,?,?,?,?,?,?,?)",[title,keywords,description,info,author,cid,text,num,time,img],function(err,data){
		if (err) {
			return "";
		}else{
...
router.get("/edit",function(req,res,next){
	...
	mssql.sql("select * from newstype order by sort desc",function(err,data){
		...
		if (err) {
			...
			// 查询修改文章对应数据
			mssql.sql("select * from news where id = "+id,function(err,data2){
				if (err) {
					return "";
				}else{
					// 加载修改页面
...
router.post("/edit",upload.single("img"),function(req,res,next){
	...
	mssql.sql("update news set cid= ? , text=? , author=? , info=? , description=? , keywords=? , title=? ,img = ? where id = ?",[cid,text,author,info,description,keywords,title,img,id],function(err,data){
		if (err) {
			return "";
		}else{
			// 判断影响行数
...
router.get("/ajax_del",function(req,res,next){
	...
	// 删除数据
	mssql.sql("delete from news where id = "+id,function(err,data){
		if (err) {
			return "";
		}else{
			if (data.affectedRows==1) {
